#!/usr/bin/env python3
"""
Gmail Popup Automation - Opens Gmail in browser with pre-filled email and PDF attachments
"""
    ...
    async def open_gmail_popup_and_fill_email(self, pdf_files, recipient_email):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                await page.wait_for_selector('[data-action="compose"]', timeout=60000)
                ...
                if pdf_files:
                    ...
                    for i, pdf_file in enumerate(pdf_files, 1):
                        try:
                            ...
                            await page.click('[data-tooltip="Attach files"]')
                            await page.wait_for_timeout(1000)
                            ...
                            try:
                                # Right-click and select "Attach files"
                                await page.click('[data-tooltip="Attach files"]', button='right')
                                await page.wait_for_timeout(1000)
                                
                                # This might open a context menu, try to click "Attach files" again
                                await page.click('[data-tooltip="Attach files"]')
                                await page.wait_for_timeout(1000)
                                ...
                                await page.wait_for_timeout(3000)
                                print(f"   ✅ Attached (retry): {Path(pdf_file).name}")
                                
                            except Exception as e2:
                                print(f"   ❌ Retry failed for {Path(pdf_file).name}: {e2}")
...
async def main():
    ...
    automation = GmailPopupAutomation()
